...
int main()
{
  ...
  for (;;) 
  {
    GPIOA_BSRR = 0x00000200; GPIOB_BSRR = 0x20001000; TIM1_CCR2 = 50000;        // "Колесо Обозрения" работает постоянно
    ...
    if(!((GPIOB_IDR & (1 << 7)) < 1))                        // если нет сигнала "Ночь"
      {
       ...
       GPIOA_BSRR = 0x00C00000;                              // выключили свет "Колеса" out2(PA6), out3(PA7) 
       
       if((timer > 1) && (timer < 200)) GPIOA_BSRR = 0x00000020; // включили свет "Осьминога" out1(PA5)                     
       else                             GPIOA_BSRR = 0x00200000; // выключили свет "Осьминога" out1(PA5)                   
      }
...
/* 
    if((timer > 1) && (timer < 900))                   // интервал 1,5 минуты
      {
       GPIOA_BSRR = 0x00000100; GPIOB_BSRR = 0x08000400; TIM1_CCR1 = 50000;     // включили двигатель M1
       if((timer > 300) && (timer < 900))                                       // ждем 30 секунд
         {
          if((GPIOA_IDR & (1 << 1)) < 1)                                        // если геркон SW1(PA2) замкнут
            {
             GPIOA_BSRR = 0x01000000; GPIOB_BSRR = 0x0C000000; TIM1_CCR1 = 0;   // выключаем двигатель M1
             timer = 900;
            }  
         }                                 
      }
